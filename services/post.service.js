...
export const getMyPosts = async (userId, options = {}) => {
    try {
        const { page = 1, limit = 10, published, sortBy = 'createdAt', sortOrder = 'desc' } = options;
        const skip = (page - 1) * limit;
...
export const updatePost = async (postId, userId, updateData) => {
    const updatedPost = await prisma.$transaction(async (tx) => {
        try {
            ...
            if (error.code === 'P2025') {
                throw new Error("Post was modified by another process. Please refresh and try again.");
            }
            throw error;
        }
    });
